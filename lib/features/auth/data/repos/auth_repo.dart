...
class AuthRepoImpl extends AuthRepo {
  ...
  @override
  Future<Either<ApiError, dynamic>> requestResetPwdToken(
      {required String email}) async {
    var request = {"email": email};
  ...
  @override
  Future<Either<ApiError, dynamic>> updatePassword(
  ...
  @override
  Future<Either<ApiError, dynamic>> getDocusignUrl(
      {required String email, required String name}) async {
    var request = {"email": email, 'name': name};

    return await catchSocketException(
            () => postReq(AuthEndPoints.getDocusignUrl, body: request))
        .then((value) => handleResponse(value));
  }

  @override
  Future<Either<ApiError, dynamic>> createTherapistApplication(
      {required TherapistApplication application}) async{

    return await catchSocketException(
            () => postReq(AuthEndPoints.therapistApplication,
                body: application.toJson()))
        .then((value) => handleResponse(value));

  }

  Future<Either> validateApprovalToken(String value) async {
    var request = {"token": value};

    return await catchSocketException(
            () => postReq(AuthEndPoints.validateApprovalToken, body: request))
        .then((value) => handleResponse(value));
  }
}
